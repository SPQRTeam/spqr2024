...
public class RobotPanel extends JPanel implements RobotStateEventListener {
    ...
    public RobotPanel(final RobotState robot) {
        ...
        SwingUtilities.invokeLater(() -> {
            ...
            for (int i = 0; i < 7; i++) { // change here if you need more space
    ...
    private void update() {
        ...
        SwingUtilities.invokeLater(() -> {
            ((JLabel) foregroundPanel.getComponent(0)).setText("Name: " + robot.name); //
            if (robot.whistleDetected == RobotState.WhistleDetector.isDetected) {
                foregroundPanel.getComponent(0).setForeground(Color.magenta);
            }
            ((JLabel) foregroundPanel.getComponent(1)).setText("Role: " + robot.role); //
            if (robot.refereePose) {
                foregroundPanel.getComponent(1).setForeground(Color.blue);
            }            
            if (robot.getPlayerNumber() == null || (robot.getLastGCRDMessage() != null && !robot.getLastGCRDMessage().playerNumValid)) {
                foregroundPanel.getComponent(2).setForeground(Color.red);
                ((JLabel) foregroundPanel.getComponent(2)).setText("Player no: " + (robot.getLastGCRDMessage() != null ? robot.getLastGCRDMessage().playerNum : "unknown"));
            } else {
                foregroundPanel.getComponent(2).setForeground(defaultColor);
                ((JLabel) foregroundPanel.getComponent(2)).setText("Player no: " + robot.getPlayerNumber());
            }
            ((JLabel) foregroundPanel.getComponent(3)).setText("Messages: " + robot.getTeamMessageCount() + " / " + robot.getGCRDMessageCount());

            ((JLabel) foregroundPanel.getComponent(4)).setText("Per second: " + df.format(robot.getTeamMessagesPerSecond()) + " / " + df.format(robot.getGCRDMessagesPerSecond()));
            if ((robot.getLastTeamMessage() != null && !robot.getLastTeamMessage().valid) || (robot.getLastGCRDMessage() != null && !robot.getLastGCRDMessage().valid)) {
                foregroundPanel.getComponent(5).setForeground(Color.red);
            } else {
                foregroundPanel.getComponent(5).setForeground(defaultColor);
            }
            ((JLabel) foregroundPanel.getComponent(5)).setText("Illegal: " + robot.getIllegalTeamMessageCount() + " (" + Math.round(robot.getIllegalTeamMessageRatio() * 100.0) + "%) / " + robot.getIllegalGCRDMessageCount() + " (" + Math.round(robot.getIllegalGCRDMessageCount() * 100.0) + "%)");
            ((JLabel) foregroundPanel.getComponent(6)).setText("Battery: " + (int) robot.batteryLevel + "% , " + robot.cpuTemperature + "°C"); //
            if (robot.batteryLevel < 34) {
                foregroundPanel.getComponent(6).setForeground(Color.red.darker());
            }
            else if (robot.batteryLevel > 33 && robot.batteryLevel < 67) {
                foregroundPanel.getComponent(6).setForeground(Color.yellow.darker());            
            }
            else {
                foregroundPanel.getComponent(6).setForeground(Color.green.darker());
            }
        });
    }
