    ...
    protected void handleMessage(final SPLTeamMessagePackage p) {
        ...
        //EB
            try {
                ...
                if (p.message.length < 320) { // since now you have 312 + 8 of size (if you include whistle attributes then you need 320 + 8)
                    System.out.println("Message NOT Entirely Received! (size: "+p.message.length+") \n");
                }
                else {
                    GameState.getInstance().receiveMessage(p.host, p.team, m); // here you pass the message
                }
