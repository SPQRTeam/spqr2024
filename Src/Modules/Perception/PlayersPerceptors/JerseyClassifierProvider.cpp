...
void JerseyClassifierProvider::detectJersey(const ObstaclesImagePercept::Obstacle& obstacleInImage, ObstaclesFieldPercept::Obstacle& obstacleOnField) const
{
                                     ...
                                     theCameraMatrix, theCameraInfo, upperInImage))
  {
    ...
    if(lowerInImage.y() >= jerseyMinYSamples)
    {
      ...
      const std::function<bool(int, int)>& isOwnPlayer = getPixelClassifier(theOwnTeamInfo.fieldPlayerColour, theOpponentTeamInfo.fieldPlayerColour, maxBrightness);
      const std::function<bool(int, int)>& isOwnKeeper = getPixelClassifier(theOwnTeamInfo.goalkeeperColour, theOpponentTeamInfo.goalkeeperColour, maxBrightness);
      const std::function<bool(int, int)>& isOpponentPlayer = getPixelClassifier(theOpponentTeamInfo.fieldPlayerColour, theOwnTeamInfo.fieldPlayerColour, maxBrightness);
      const std::function<bool(int, int)>& isOpponentKeeper = getPixelClassifier(theOpponentTeamInfo.goalkeeperColour, theOwnTeamInfo.goalkeeperColour, maxBrightness);
      float ownPixels = 0;
          ...
          y += yStep, left = std::max(left + xyStep, 0.f), right = std::min(right + xyStep, static_cast<float>(theCameraInfo.width)))
      {
        ...
        for(float x = left; x < right; x += xStep)
        {
          ...
          DOT("module:JerseyClassifierProvider:jerseyWeights", static_cast<int>(x), static_cast<int>(y),
              ...
              ColorRGBA(static_cast<unsigned  char>(240 - 240 * weight), static_cast<unsigned  char>(240 * weight), static_cast<unsigned  char>(240 * weight), 220));
          if(isOwnPlayer(static_cast<int>(x), static_cast<int>(y)) || isOwnKeeper(static_cast<int>(x), static_cast<int>(y)))
          {
            ownPixels += weight;
            DOT("module:JerseyClassifierProvider:jersey", static_cast<int>(x), static_cast<int>(y), ColorRGBA::yellow, ColorRGBA::yellow);
          }
          else if(isOpponentPlayer(static_cast<int>(x), static_cast<int>(y)) || isOpponentKeeper(static_cast<int>(x), static_cast<int>(y)))
          {
