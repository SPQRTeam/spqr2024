...
// DANIELE E FRANCESCO 2023 -> FIX UNSTABLE

#define DISREGARD_EXTRA_PHASES true
...
std::unique_ptr<MotionPhase> WalkPhase::createNextPhase(const MotionPhase& defaultNextPhase) const
{
  ...
  if(
    !DISREGARD_EXTRA_PHASES
    && defaultNextPhase.type != MotionPhase::walk
    && walkState != stopping
    && walkState != standing
    && isStandingPossible(forwardL, forwardR, lastPrevSideL, lastPrevSideR, turnRL, walkStepAdjustment.lastLeftAdjustmentX, walkStepAdjustment.lastRightAdjustmentX)
  )
