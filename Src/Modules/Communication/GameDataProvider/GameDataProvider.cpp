...
bool GameDataProvider::sendAliveMessage()
{
  ...
  returnPacket.pose[2] = theRobotPose.rotation;         // x,y,theta

  // ball information
  returnPacket.ballAge = theFrameInfo.getTimeSince(theBallModel.timeWhenLastSeen);         // seconds since this robot last saw the ball. -1.f if we haven't seen it
