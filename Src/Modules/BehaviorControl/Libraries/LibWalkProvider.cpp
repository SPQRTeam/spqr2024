...
void LibWalkProvider::update(LibWalk& libWalk)
{
  ...
  DECLARE_DEBUG_DRAWING3D("behavior:LibWalkProvider:obstacles3D", "field");
...
MotionRequest::ObstacleAvoidance LibWalkProvider::calcObstacleAvoidance(const Pose2f& originalTarget, bool rough, bool disableObstacleAvoidance)
{
  ...
  if(!obstacleAvoidance.path.empty()){
    Vector2f path_target_rel = obstacleAvoidance.path[0].obstacle.center;
    Vector2f path_target_glob = theRobotPose * path_target_rel;
    float path_target_radius = obstacleAvoidance.path[0].obstacle.radius;
    
    Geometry::Line to_target = Geometry::Line(theRobotPose.translation, path_target_glob-theRobotPose.translation);
    
    CYLINDER3D("behavior:LibWalkProvider:obstacles3D", path_target_glob.x(), path_target_glob.y(), 0, 0, 0, 0, 5, 100, ColorRGBA::orange);
    LINE3D("behavior:LibWalkProvider:obstacles3D", theRobotPose.translation.x(), theRobotPose.translation.y(), 0, path_target_glob.x(), path_target_glob.y(), 0, 5, ColorRGBA::green);

    Obstacle nopp = obstacles[0];
    for(auto obs:obstacles){
      Vector2f obs_center = obs.center;
      if((obs_center).norm() < (nopp.center).norm()){
        nopp = obs;
      }
    }
    
    Vector2f nopp_glob = theRobotPose * nopp.center;
    Vector2f nopp_left_glob = theRobotPose * nopp.outerLeft;
    Vector2f nopp_right_glob = theRobotPose * nopp.outerRight;

    Geometry::Circle obs_circle = Geometry::Circle(nopp_glob, obstacleRadius);
    Vector2f int1;
    Vector2f int2;
    Vector2f intersection;
    int num_of_intersections = Geometry::getIntersectionOfLineAndCircle(to_target, obs_circle, int1, int2);
    if(num_of_intersections==1){
      intersection = int1;
    }
    else if(num_of_intersections==2){
      intersection = (int1-path_target_glob).norm() < (int2-path_target_glob).norm() ? int1 : int2;
    }
    if(num_of_intersections>0){
      Vector2f new_target = (nopp_left_glob-targetOnField).norm() < (nopp_right_glob-targetOnField).norm() ? nopp_left_glob : nopp_right_glob;
      obstacleAvoidance.path[0].obstacle.center = theRobotPose.inversePose * new_target;
    }
  }
...
void LibWalkProvider::calculateObstacles(const Pose2f& target, const Vector2f& targetOnField, const bool rough, const bool disableObstacleAvoidance)
{
      ...
      && std::abs(targetOnField.y()) < theFieldDimensions.yPosLeftSideline))
  {
    if(theRobotPose.translation.y() > leftOpponentCorner.y() - fieldBorderAvoidanceDistance)
    {
      ...
      addObstacle(obstaclePos, obstacleRadius, fieldBorderAvoidanceMinRadius, fieldBorderAvoidanceMaxRadius, fieldBorderAvoidanceDistance);
    }
  ...
  if(rough)
  {
    ...
    for(Obstacle& o : obstacles)
    {
      ...
      if((t < 0.f || t > 1.f || (o.left.squaredNorm() > targetSqrDistance && o.right.squaredNorm() > targetSqrDistance)) && 
         (o.center.norm() > obstacleRadius || target.translation.norm() < 300)) //** Valerio
        o.active = false;
  ...
  if(!theRobotInfo.isGoalkeeper() && theLibTeammates.nonKeeperTeammatesInOwnPenaltyArea >= 2)
  {
    if(theLibPosition.isNearOwnPenaltyArea(theRobotPose.translation, penaltyAreaAvoidanceDistance, penaltyAreaAvoidanceDistance))
    {
      ...
      obstaclePos = theRobotPose.inversePose * obstaclePos;
      addObstacle(obstaclePos, obstacleRadius, penaltyAreaAvoidanceMinRadius, penaltyAreaAvoidanceMaxRadius, penaltyAreaAvoidanceDistance);
    }
  }
  ...
  for(const Obstacle& o : obstacles)
  {
    ...
    auto outerLeft = theRobotPose * o.outerLeft;
    auto outerRight = theRobotPose * o.outerRight;
    LINE3D("behavior:LibWalkProvider:obstacles3D", left.x(), left.y(), 0, right.x(), right.y(), 0, 2.5, color);
    LINE3D("behavior:LibWalkProvider:obstacles3D", left.x(), left.y(), 0, outerLeft.x(), outerLeft.y(), 0, 2.5, color);
    LINE3D("behavior:LibWalkProvider:obstacles3D", right.x(), right.y(), 0, outerRight.x(), outerRight.y(), 0, 2.5, color);
    CIRCLE3D("behavior:LibWalkProvider:obstacles3D", (theRobotPose*o.center).x(), (theRobotPose*o.center).y(), 0, 450, 5, color);
  }
                                  ...
                                  const float minAvoidanceRadius, const float maxAvoidanceRadius, const float avoidanceDistance)
{
  ...
  //** New implementation (Valerio)
  float min_angle = 10;
  float max_angle = 45;
  float normalized_radius = (radius-minAvoidanceRadius)/(maxAvoidanceRadius-minAvoidanceRadius);
  const Angle angle = (90+min_angle + normalized_radius*(max_angle-min_angle))*M_PI/180;
  const Vector2f outerLeft = left + radius*Vector2f(cos(angle + left.angle()), sin(angle + left.angle()));
  const Vector2f outerRight = right + radius*Vector2f(cos(-angle + right.angle()), sin(-angle + right.angle()));
  const Angle angleLeft = outerLeft.angle();
  const Angle angleRight = outerRight.angle();
