...
void LibTeamProvider::update(LibTeam& libTeam)
{
  ...
  libTeam.timeSinceBallWasSeen = theFrameInfo.getTimeSince(theTeamBallModel.timeWhenLastSeen);
}
...
Pose2f LibTeamProvider::getKeeperPose() const
{
  ...
  {
    if(teammate.status != Teammate::PENALIZED && teammate.roleIsGoalkeeper())
    {
...
Pose2f LibTeamProvider::getStrikerPose() const
{
  ...
  for(auto const& teammate : theTeamData.teammates)
  {
    if(teammate.status != Teammate::PENALIZED && teammate.rolePlaysTheBall())
    {
      return teammate.theRobotPose;
    }
  }
  ...
  for(auto const& teammate : theTeamData.teammates)
  {
    if(teammate.number == player)
    {
      if(teammate.status != Teammate::PENALIZED) {
        ...
        FAIL("BehaviorStatus non in rete");
        break;
      }
      else
        break;
