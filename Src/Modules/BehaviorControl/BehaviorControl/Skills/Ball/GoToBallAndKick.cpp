...
class GoToBallAndKickImpl : public GoToBallAndKickImplBase
{
  ...
  {
    if(kickType == KickInfo::forwardFastRight || kickType == KickInfo::forwardFastLeft || kickType == KickInfo::ottoMetriRightKick || kickType == KickInfo::ottoMetriLeftKick || kickType == KickInfo::ottoMetriLooseRightKick || kickType == KickInfo::ottoMetriLooseLeftKick || kickType == KickInfo::lobKick || kickType == KickInfo::kickTest)
    {
      // This is only dummy code.
      // TODO: Daniele-Vincenzo
      if(length < 1000.f)
        return 0.f;
      else if(length > 6000.f)
        return 1.f;
      else
        return (length - 1000.f) / 6000.f;
    }
    else
    {
