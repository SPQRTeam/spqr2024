...
// may lag behind by one frame depending on execution order, whatever.
// this may want a proper, separate provider.
int pippo(float x) {
  short x_short = (short) x;
  int x_int = x_short;
  return x_int;
}
void ObstacleModelProvider::update(DiscretizedObstacleModel& discomod) {
  discomod.obstacles.clear();

  for (const Obstacle& obstacle : theObstacleModel.obstacles) {
    DiscretizedObstacle disco;
    #if DISCOMOD_VER==0
    disco.center = obstacle.center;
    disco.covariance = obstacle.covariance;
    disco.left = obstacle.left;
    disco.right = obstacle.right;
    disco.velocity = obstacle.velocity;
    #elif DISCOMOD_VER==1
    disco.center = obstacle.center.cast<short>();
    disco.covariance = obstacle.covariance.cast<short>();
    disco.left = obstacle.left.cast<short>();
    disco.right = obstacle.right.cast<short>();
    disco.velocity = obstacle.velocity.cast<short>();
    ...
    #elif DISCOMOD_VER==2
    disco.covariance = obstacle.covariance.cast<short>();
    disco.center = (pippo(obstacle.center.x()) << 16) | ((short) obstacle.center.y());
    disco.left = (pippo(obstacle.left.x()) << 16) | ((short) obstacle.left.y());
    disco.right = (pippo(obstacle.right.x()) << 16) | ((short) obstacle.right.y());
    disco.velocity = (pippo(obstacle.velocity.x()) << 16) | ((short) obstacle.velocity.y());
    #elif DISCOMOD_VER==3
    disco.covariance = obstacle.covariance;
    disco.center = obstacle.center.cast<short>();
    #elif DISCOMOD_VER==4
    disco.center = obstacle.center;
    #endif
    disco.lastSeen = obstacle.lastSeen;
    disco.type = obstacle.type;
    discomod.obstacles.push_back(disco);
  }
}
