...
bool RoboCupCtrl::compile()
{
  ...
  if(teamColors != nullptr)
  {
    ...
    firstFieldPlayerColour = static_cast<Settings::TeamColor>(TypeRegistry::getEnumValue(typeid(Settings::TeamColor).name(), baseNameFirstTeamColor));
    secondFieldPlayerColour = static_cast<Settings::TeamColor>(TypeRegistry::getEnumValue(typeid(Settings::TeamColor).name(), baseNameSecondTeamColor));
    firstGoalkeeperColour = Settings::TeamColor::red;
    secondGoalkeeperColour = Settings::TeamColor::orange;
  }
  if(firstFieldPlayerColour == static_cast<Settings::TeamColor>(-1))
    firstFieldPlayerColour = Settings::TeamColor::black;
    firstGoalkeeperColour = Settings::TeamColor::red;
  if(secondFieldPlayerColour == static_cast<Settings::TeamColor>(-1))
    secondFieldPlayerColour = Settings::TeamColor::blue;
    secondGoalkeeperColour = Settings::TeamColor::orange;
  //gameController.setTeamInfos(firstFieldPlayerColour, secondFieldPlayerColour, firstGoalkeeperColour, secondGoalkeeperColour);
  gameController.setTeamInfos(Settings::TeamColor::red, Settings::TeamColor::blue, Settings::TeamColor::black, Settings::TeamColor::orange);
  ...
  for(unsigned currentRobot = 0, count = application->getObjectChildCount(*group); currentRobot < count; ++currentRobot)
  {
    ...
    robots.push_back(new ControllerRobot(Settings("Nao", "Nao",
                                                  firstTeam ? 1 : 2,
                                                  firstTeam ? firstFieldPlayerColour : secondFieldPlayerColour,
                                                  firstTeam ? firstGoalkeeperColour : secondGoalkeeperColour,
                                                  SimulatedRobot::getNumber(robot) - (firstTeam ? 0 : SPQR_MAX_PLAYER_NUMBER),
