...
#define MAX_NTP_MESSAGES 6   // tested after observing a few minutes of sim
...
void BNTP::operator>>(BHumanMessage& m) const
{
  ...
  // int size_before = m.theBHumanStandardMessage.sizeOfBHumanMessage();

  for(const auto& pair : receivedNTPRequests)
  {
    if (m.theBHumanStandardMessage.ntpMessages.size() >= MAX_NTP_MESSAGES) {
      OUTPUT_WARNING("Attempted to send more than " << MAX_NTP_MESSAGES << " NTP messages. (Extra message(s) skipped.)");
      break;
    }
  ...
  // int size_after = m.theBHumanStandardMessage.sizeOfBHumanMessage();
  // OUTPUT_TEXT("ntp-mess-" << (int) m.theBHumanStandardMessage.ntpMessages.size() << " bytes-" << (size_after-size_before));
}
