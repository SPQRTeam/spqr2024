...
bool Teammate::handleMessage(InMessage& message)
{
  switch(message.getMessageID())
  {
    ...
    //   // the actual reading is in the PlayerRole files in order to keep reading as close as possible to writing.
    //   role = PlayerRole::specialHandlingOfArbitraryMessage(message);
    //   return true;
    default:
      return false;
  }
...
void TeamData::draw() const
{
  ...
  for(auto const& teammate : teammates)
  {
    ...
    DRAW_TEXT("representation:TeamData", rPos.x() + 100, rPos.y() - 150, 100,
             ColorRGBA::black, TypeRegistry::getEnumName(teammate.role));

    // Time to reach ball (NON FUNZIONA SE TEAMBEHAVIORSTATUS NON È IN RETE)
    int ttrb = teammate.rolePlaysTheBall()
    ...
    // Line from Robot to WalkTarget (NON FUNZIONA SE BEHAVIORSTATUS NON È IN RETE)
