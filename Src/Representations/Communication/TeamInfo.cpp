...
{
  if(number < SPQR_MAX_PLAYER_NUMBER)
    return number;

  for(unsigned int i = 0; i < SPQR_MAX_PLAYER_NUMBER - 1; i++)
...
{
  fieldPlayerColour = Global::settingsExist() ? Global::getSettings().fieldPlayerColour : TEAM_BLACK;
  goalkeeperColour = Global::settingsExist() ? Global::getSettings().goalkeeperColour : TEAM_ORANGE;
}
...
{
  fieldPlayerColour = 1 ^ (Global::settingsExist() ? Global::getSettings().fieldPlayerColour : TEAM_RED);
}
